...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagstart_jing)
    {
        info->event[info->num] = EVENT_start_jing;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuestart_jing,(uint8_t *)&issuedData[byteFieldOffset],start_jing_LEN);
        byteFieldOffset += start_jing_LEN;
        dataPoints->valuestart_jing = gizX2YFloat(start_jing_RATIO,  start_jing_ADDITION, (float)(exchangeBytes(elongateP0FormTmp.devStatus.valuestart_jing)));
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagstart_wei)
    {
        info->event[info->num] = EVENT_start_wei;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuestart_wei,(uint8_t *)&issuedData[byteFieldOffset],start_wei_LEN);
        byteFieldOffset += start_wei_LEN;
        dataPoints->valuestart_wei = gizX2YFloat(start_wei_RATIO,  start_wei_ADDITION, (float)(exchangeBytes(elongateP0FormTmp.devStatus.valuestart_wei)));
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagend_jing)
    {
        info->event[info->num] = EVENT_end_jing;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueend_jing,(uint8_t *)&issuedData[byteFieldOffset],end_jing_LEN);
        byteFieldOffset += end_jing_LEN;
        dataPoints->valueend_jing = gizX2YFloat(end_jing_RATIO,  end_jing_ADDITION, (float)(exchangeBytes(elongateP0FormTmp.devStatus.valueend_jing)));
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagend_wei)
    {
        info->event[info->num] = EVENT_end_wei;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueend_wei,(uint8_t *)&issuedData[byteFieldOffset],end_wei_LEN);
        byteFieldOffset += end_wei_LEN;
        dataPoints->valueend_wei = gizX2YFloat(end_wei_RATIO,  end_wei_ADDITION, (float)(exchangeBytes(elongateP0FormTmp.devStatus.valueend_wei)));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuewarn_flag != cur->valuewarn_flag)
    {
        GIZWITS_LOG("valuewarn_flag Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwarn_flag = 1;
        ret = 1;
    }
    if(last->valuestart_jing != cur->valuestart_jing)
    {
        GIZWITS_LOG("valuestart_jing Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagstart_jing = 1;
        ret = 1;
    }
    if(last->valuestart_wei != cur->valuestart_wei)
    {
        GIZWITS_LOG("valuestart_wei Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagstart_wei = 1;
        ret = 1;
    }
    if(last->valueend_jing != cur->valueend_jing)
    {
        GIZWITS_LOG("valueend_jing Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagend_jing = 1;
        ret = 1;
    }
    if(last->valueend_wei != cur->valueend_wei)
    {
        GIZWITS_LOG("valueend_wei Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagend_wei = 1;
        ret = 1;
    }

    if(last->valuejingdu != cur->valuejingdu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuejingdu Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagjingdu = 1;
            ret = 1;
        }
    }
    if(last->valueweidu != cur->valueweidu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueweidu Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagweidu = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagwarn_flag)
    {
        gizVarlenCompressValue(bitFieldOffset,warn_flag_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuewarn_flag);
        bitFieldOffset += warn_flag_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagjingdu)
    {
        devStatusTmp.valuejingdu = exchangeBytes(gizY2XFloat(jingdu_RATIO,  jingdu_ADDITION, dataPoints->valuejingdu));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuejingdu,jingdu_LEN);
        byteFieldOffset += jingdu_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagweidu)
    {
        devStatusTmp.valueweidu = exchangeBytes(gizY2XFloat(weidu_RATIO,  weidu_ADDITION, dataPoints->valueweidu));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueweidu,weidu_LEN);
        byteFieldOffset += weidu_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagstart_jing)
    {
        devStatusTmp.valuestart_jing = exchangeBytes(gizY2XFloat(start_jing_RATIO,  start_jing_ADDITION, dataPoints->valuestart_jing));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuestart_jing,start_jing_LEN);
        byteFieldOffset += start_jing_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagstart_wei)
    {
        devStatusTmp.valuestart_wei = exchangeBytes(gizY2XFloat(start_wei_RATIO,  start_wei_ADDITION, dataPoints->valuestart_wei));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuestart_wei,start_wei_LEN);
        byteFieldOffset += start_wei_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagend_jing)
    {
        devStatusTmp.valueend_jing = exchangeBytes(gizY2XFloat(end_jing_RATIO,  end_jing_ADDITION, dataPoints->valueend_jing));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueend_jing,end_jing_LEN);
        byteFieldOffset += end_jing_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagend_wei)
    {
        devStatusTmp.valueend_wei = exchangeBytes(gizY2XFloat(end_wei_RATIO,  end_wei_ADDITION, dataPoints->valueend_wei));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueend_wei,end_wei_LEN);
        byteFieldOffset += end_wei_LEN;
    }
