...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuelow_hearts != cur->valuelow_hearts)
    {
        GIZWITS_LOG("valuelow_hearts Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaglow_hearts = 1;
        ret = 1;
    }
    if(last->valuehigh_hearts != cur->valuehigh_hearts)
    {
        GIZWITS_LOG("valuehigh_hearts Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaghigh_hearts = 1;
        ret = 1;
    }
    if(last->valuelow_blood_pressure != cur->valuelow_blood_pressure)
    {
        GIZWITS_LOG("valuelow_blood_pressure Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaglow_blood_pressure = 1;
        ret = 1;
    }
    if(last->valuehigh_blood_pressure != cur->valuehigh_blood_pressure)
    {
        GIZWITS_LOG("valuehigh_blood_pressure Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaghigh_blood_pressure = 1;
        ret = 1;
    }
    ...
    if(last->valuespo2 != cur->valuespo2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagspo2 = 1;
            ret = 1;
        }
    }
    if(last->valueSystolic != cur->valueSystolic)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueSystolic Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagSystolic = 1;
            ret = 1;
        }
    }
    if(last->valueDiastole != cur->valueDiastole)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueDiastole Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagDiastole = 1;
            ret = 1;
        }
    }
    if(last->valuehearts != cur->valuehearts)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuehearts Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flaghearts = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flaglow_hearts)
    {
        gizVarlenCompressValue(bitFieldOffset,low_hearts_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuelow_hearts);
        bitFieldOffset += low_hearts_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaghigh_hearts)
    {
        gizVarlenCompressValue(bitFieldOffset,high_hearts_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuehigh_hearts);
        bitFieldOffset += high_hearts_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaglow_blood_pressure)
    {
        gizVarlenCompressValue(bitFieldOffset,low_blood_pressure_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuelow_blood_pressure);
        bitFieldOffset += low_blood_pressure_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaghigh_blood_pressure)
    {
        gizVarlenCompressValue(bitFieldOffset,high_blood_pressure_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuehigh_blood_pressure);
        bitFieldOffset += high_blood_pressure_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagspo2)
    {
        devStatusTmp.valuespo2 = gizY2X(spo2_RATIO,  spo2_ADDITION, dataPoints->valuespo2); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuespo2,spo2_LEN);
        byteFieldOffset += spo2_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagSystolic)
    {
        devStatusTmp.valueSystolic = gizY2X(Systolic_RATIO,  Systolic_ADDITION, dataPoints->valueSystolic); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueSystolic,Systolic_LEN);
        byteFieldOffset += Systolic_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagDiastole)
    {
        devStatusTmp.valueDiastole = gizY2X(Diastole_RATIO,  Diastole_ADDITION, dataPoints->valueDiastole); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueDiastole,Diastole_LEN);
        byteFieldOffset += Diastole_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flaghearts)
    {
        devStatusTmp.valuehearts = exchangeBytes(gizY2X(hearts_RATIO,  hearts_ADDITION, dataPoints->valuehearts)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuehearts,hearts_LEN);
        byteFieldOffset += hearts_LEN;
    }
    ...
    {
        devStatusTmp.valuetemperature = exchangeBytes(gizY2XFloat(temperature_RATIO,  temperature_ADDITION, dataPoints->valuetemperature));     
